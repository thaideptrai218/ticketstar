...
| Question | Decision |
|---|---|
| Refresh token rotation | **Yes** — rotation + reuse detection (revoke all sessions on reuse) |
| Payment provider | **SePay (VietQR)** — redirect/QR flow, not Stripe Elements |
| Offline check-in | **No** — online only, no PWA/IndexedDB needed |
| Multi-tenant isolation | **Strictly isolated** — organizer dashboard filters by OrganizerId; no cross-organizer visibility |
| Auth methods | **Google OAuth + Magic Link** — no username/password |
